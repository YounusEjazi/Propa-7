...
const ExerciseDetails = () => {
  ...
  const [boxes, setBoxes] = useState({
    box1: [
      ...
      { id: 8, src: '/Tragwerkelemente/le/Fachwerkträger.png', name: 'Fachwerkträger' },
      { id: 9, src: '/Tragwerkelemente/le/Rahmenträger.png', name: 'Rahmenträger' },
      { id: 10, src: '/Tragwerkelemente/le/Eingespannter Rahmen.png', name: 'Eingespannter Rahmen' },
      { id: 11, src: '/Tragwerkelemente/le/Zweigelenkrahmen.png', name: 'Zweigelenkrahmen' }
    ],
    box2: [
      ...
      { id: 15, src: '/Tragwerkelemente/fügung/gelenkigefügung.png', name: 'Gelenkigefügung' }
    ],
    box3: [
      { id: 16, src: '/Tragwerkelemente/auflager/fest.png', name: 'Fest' },
      { id: 17, src: '/Tragwerkelemente/auflager/gespannt.png', name: 'Gespannnt' },
      { id: 18, src: '/Tragwerkelemente/auflager/verschieblich.png', name: 'Verschieblich' }
    ],
    box4: [],
  });
  ...
  const handleElementClick = (item) => {
    ...
    if (predefinedArea) {
      ...
      alert('The selected area does not match the predefined area for the item.');
  ...
  const handleSaveDetails = () => {
    fetch(`http://localhost:3000/update-exercise/${id}`, {
      ...
      .then(data => {
        if (data.status === 'ok') {
          alert('Exercise details updated successfully');
          setIsEditing(false);
        } else {
        ...
        <div className="row">
          <div className="column">
            <div style={{ position: 'relative' }}>
              <SelectableSection predefinedAreas={predefinedAreas} onSelect={handleSelectArea} onUnlock={handleUnlock} userType={user.userType}>
                <div className="img-container">
                  <img src={exercise.img} alt={exercise.title} style={{ maxWidth: '100%', height: 'auto' }} />
                </div>
              </SelectableSection>
              {selectedSection && (
                <div
              ...
              {user.userType === 'Admin' && (
                <div>
                  <Button onClick={() => setIsEditing(!isEditing)} className="inline-btn">
                  ...
                  {isEditing && (
                    <div>
                      ...
                      />
                      <Button onClick={handleSaveArea} className="inline-btn" block>
                        Save Selected Area
                      </Button>
                    </div>
                  )}
        ...
        <div className="box-container1">
          <div className="box-header">
            ...
            <div className="droppable-box" id="box1">
              {boxes.box1.map((img) => (
                <div key={img.id} id={`box1-${img.id}`} className="image-wrapper" onClick={() => handleElementClick(img)}>
                  <div className="circular-border">
                    <img src={img.src} alt={img.name} className="draggable-image" />
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="box-header">
            ...
            <div className="droppable-box" id="box2">
              {boxes.box2.map((img) => (
                <div key={img.id} id={`box2-${img.id}`} className="image-wrapper" onClick={() => handleElementClick(img)}>
                  <div className="circular-border">
                    <img src={img.src} alt={img.name} className="draggable-image" />
                  </div>
                </div>
          ...
          <div className="box-header">
            ...
            <div className="droppable-box" id="box3">
              {boxes.box3.map((img) => (
                <div key={img.id} id={`box3-${img.id}`} className="image-wrapper" onClick={() => handleElementClick(img)}>
                  <div className="circular-border">
                    <img src={img.src} alt={img.name} className="draggable-image" />
                  </div>
                </div>
          ...
          <div className="box-header">
            ...
            <div className="droppable-box" id="box4">
              {boxes.box4.map((img) => (
                <div key={img.id} id={`box4-${img.id}`} className="image-wrapper" onClick={() => handleElementClick(img)}>
                  <div className="circular-border">
                    <img src={img.src} alt={img.name} className="draggable-image" />
                  </div>
                </div>
              ...
              <div className="tip-box">
                ...
                {incorrectAttempts >= 10 && (
                  <Button className="tip-button" onClick={() => navigate('/help-page')}>
                    Get Help
                  </Button>
                )}
              </div>
